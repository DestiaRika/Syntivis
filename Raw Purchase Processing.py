...
# Merge
        ...
        # Filter yang belum match di tahap 3
        ...
        if len(df_grpo_timb_tidak_ada_kedua) > 0:
            st.write(f"🔍 Data GRPO yang belum match di tahap 3: {len(df_grpo_timb_tidak_ada_kedua)} baris")
            ...
            # Simpan hasil tahap 4 untuk dipakai di tahap 5
            df_matched_tahap3 = df_grpo_timbangan_ketiga[df_grpo_timbangan_ketiga['Doc Number_TIMB'].notna()].copy()
            ...
            df_grpo_timbangan_ketiga = df_grpo_timbangan_kedua.copy()
            matched_third = 0
            df_matched_tahap3 = pd.DataFrame()  # Empty dataframe


        # ========================================================================
        # TAHAP 5: MATCHING KEEMPAT (INSTRUCTION NUMBER + QUANTITY SUM)
        # ========================================================================
        st.write("\nTAHAP 5: Matching Keempat (Instruction Number + Quantity Sum)")
        st.write("=" * 70)

        # Filter yang belum match di tahap 4
        df_grpo_timb_tidak_ada_ketiga = df_grpo_timbangan_ketiga[df_grpo_timbangan_ketiga['Doc Number_TIMB'].isna()].copy()

        if len(df_grpo_timb_tidak_ada_ketiga) > 0:
            st.write(f"🔍 Data GRPO yang belum match di tahap 4: {len(df_grpo_timb_tidak_ada_ketiga)} baris")
            
            # Reload timbangan untuk matching keempat
            df_timbangan_keempat = pd.read_excel(timbangan)
            df_timbangan_keempat.columns = df_timbangan_keempat.columns.str.strip()
            
            # ========== STANDARDISASI TIMBANGAN KEEMPAT ==========
            if "Doc Number" in df_timbangan_keempat.columns:
                df_timbangan_keempat["Doc Number"] = df_timbangan_keempat["Doc Number"].astype(str).str.strip().str.upper()
            
            if "Weight Difference" in df_timbangan_keempat.columns:
                df_timbangan_keempat["Weight Difference"] = df_timbangan_keempat["Weight Difference"].apply(format_number)
            
            # ========== STANDARDISASI GRPO (yang belum match) ==========
            df_grpo_timb_tidak_ada_ketiga.columns = df_grpo_timb_tidak_ada_ketiga.columns.str.strip()
            
            if "Instruction Number_GRPO" in df_grpo_timb_tidak_ada_ketiga.columns:
                df_grpo_timb_tidak_ada_ketiga["Instruction Number_GRPO"] = df_grpo_timb_tidak_ada_ketiga["Instruction Number_GRPO"].astype(str).str.strip().str.upper()
            
            # ========== AGGREGASI QUANTITY BERDASARKAN INSTRUCTION NUMBER ==========
            # Group by Instruction Number dan sum quantity
            key_cols_tahap5 = ["Instruction Number_GRPO"]
            
            if "Quantity_GRPO" in df_grpo_timb_tidak_ada_ketiga.columns:
                # Pastikan Quantity numeric
                df_grpo_timb_tidak_ada_ketiga["Quantity_GRPO"] = pd.to_numeric(
                    df_grpo_timb_tidak_ada_ketiga["Quantity_GRPO"], errors="coerce"
                ).fillna(0)
                
                # Sum Quantity per Instruction Number
                df_grpo_timb_tidak_ada_ketiga["qty_sum_fmt"] = (
                    df_grpo_timb_tidak_ada_ketiga
                    .groupby(key_cols_tahap5, dropna=False)["Quantity_GRPO"]
                    .transform("sum")
                )
                
                # Format quantity sum
                df_grpo_timb_tidak_ada_ketiga["qty_sum_fmt"] = df_grpo_timb_tidak_ada_ketiga["qty_sum_fmt"].apply(format_number)
            
            # ========== BUAT MERGED KEY KEEMPAT ==========
            # TIMBANGAN: Doc Number_Weight Difference
            df_timbangan_keempat["merged_key_instruction"] = (
                df_timbangan_keempat["Doc Number"].apply(clean_decimal_zero) + "_" +
                df_timbangan_keempat["Weight Difference"].apply(clean_decimal_zero)
            )
            
            # GRPO: Instruction Number_qty_sum_fmt
            df_grpo_timb_tidak_ada_ketiga["merged_key_instruction"] = (
                df_grpo_timb_tidak_ada_ketiga["Instruction Number_GRPO"].apply(clean_decimal_zero) + "_" +
                df_grpo_timb_tidak_ada_ketiga["qty_sum_fmt"].apply(clean_decimal_zero)
            )
            
            st.write("🔍 Preview merged_key_instruction TIMBANGAN (5 baris pertama):")
            st.dataframe(df_timbangan_keempat[["Doc Number", "Weight Difference", "merged_key_instruction"]].head(5))
            
            st.write("\n🔍 Preview merged_key_instruction GRPO (5 baris pertama):")
            st.dataframe(df_grpo_timb_tidak_ada_ketiga[["Instruction Number_GRPO", "qty_sum_fmt", "merged_key_instruction"]].head(5))
            
            # ========== MERGE KEEMPAT ==========
            # Clean merged_key_instruction
            df_timbangan_keempat['merged_key_instruction'] = df_timbangan_keempat['merged_key_instruction'].astype(str).str.strip()
            df_grpo_timb_tidak_ada_ketiga['merged_key_instruction'] = df_grpo_timb_tidak_ada_ketiga['merged_key_instruction'].astype(str).str.strip()
            
            # Rename kolom timbangan
            cols_to_rename_keempat = {col: f"{col}_TIMB" for col in df_timbangan_keempat.columns if col != 'merged_key_instruction'}
            df_timbangan_keempat = df_timbangan_keempat.rename(columns=cols_to_rename_keempat)
            
            # Drop kolom _TIMB dari tahap sebelumnya untuk hindari duplikasi
            cols_to_drop = [col for col in df_grpo_timb_tidak_ada_ketiga.columns if col.endswith('_TIMB')]
            if cols_to_drop:
                st.write(f"🗑️ Menghapus {len(cols_to_drop)} kolom _TIMB dari tahap sebelumnya")
                df_grpo_timb_tidak_ada_ketiga = df_grpo_timb_tidak_ada_ketiga.drop(columns=cols_to_drop)
            
            # Merge
            df_grpo_timbangan_keempat = pd.merge(
                df_grpo_timb_tidak_ada_ketiga,
                df_timbangan_keempat, 
                on='merged_key_instruction', 
                how='left'
            )
            
            # ========== HAPUS DUPLIKAT _GRPO SETELAH MERGE TAHAP 5 ==========
            st.write("🔍 Memeriksa duplikat Doc Number_GRPO setelah merge Tahap 5...")
            if 'Doc Number_GRPO' in df_grpo_timbangan_keempat.columns:
                before_drop = len(df_grpo_timbangan_keempat)
                df_grpo_timbangan_keempat = df_grpo_timbangan_keempat.drop_duplicates(subset=['Doc Number_GRPO'], keep='first')
                after_drop = len(df_grpo_timbangan_keempat)
                removed = before_drop - after_drop
                
                if removed > 0:
                    st.warning(f"⚠️ Ditemukan {removed} duplikat Doc Number_GRPO di Tahap 5. Duplikat telah dihapus.")
                else:
                    st.success("✓ Tidak ada duplikat Doc Number_GRPO di Tahap 5")
            
            # Hitung yang match di tahap keempat
            matched_fourth = df_grpo_timbangan_keempat['Doc Number_TIMB'].notna().sum()
            st.write(f"\n✅ **Matching Keempat Berhasil**: {matched_fourth} baris")
            
            st.success("✓ TAHAP 5 selesai")
            st.write("=" * 70)
            
        else:
            st.info("✓ Semua data sudah match di Tahap 2, 3, dan 4. Tidak ada yang perlu diproses di Tahap 5.")
            df_grpo_timbangan_keempat = df_grpo_timbangan_ketiga.copy()
            matched_fourth = 0


        # ========================================================================
        # GABUNGKAN HASIL AKHIR DARI SEMUA TAHAP
        # ========================================================================
        st.write("\n" + "=" * 70)
        st.write("📦 MENGGABUNGKAN HASIL DARI SEMUA TAHAP")
        st.write("=" * 70)

        # Ambil yang match di setiap tahap
        df_matched_tahap2_final = df_grpo_timbangan_kedua[df_grpo_timbangan_kedua['Doc Number_TIMB'].notna()].copy()

        # Gabungkan semua hasil
        df_grpo_timbangan_final = pd.concat([
            df_grpo_timb_ada,              # Tahap 2 (Vendor+Item+Qty+Date)
            df_matched_tahap2_final,       # Tahap 3 (License+Qty+Date) yang matched
            df_matched_tahap3,             # Tahap 4 (License+Qty, tanpa Date) yang matched
            df_grpo_timbangan_keempat      # Tahap 5 (semua, termasuk matched dan tidak)
        ], ignore_index=True)

        # Hapus duplikat berdasarkan Doc Number_GRPO
        if 'Doc Number_GRPO' in df_grpo_timbangan_final.columns:
            before_final = len(df_grpo_timbangan_final)
            df_grpo_timbangan_final = df_grpo_timbangan_final.drop_duplicates(subset=['Doc Number_GRPO'], keep='first')
            after_final = len(df_grpo_timbangan_final)
            removed_final = before_final - after_final
            
            if removed_final > 0:
                st.warning(f"⚠️ Ditemukan {removed_final} duplikat Doc Number_GRPO di hasil final. Duplikat telah dihapus.")

        # Hitung total matched
        total_matched = len(df_grpo_timb_ada) + len(df_matched_tahap2_final) + matched_third + matched_fourth

        st.write(f"\n📊 **RINGKASAN FINAL:**")
        st.write(f"   - Tahap 2 (Vendor+Item+Qty+Date): {len(df_grpo_timb_ada)} baris")
        st.write(f"   - Tahap 3 (License+Qty+Date): {len(df_matched_tahap2_final)} baris")
        st.write(f"   - Tahap 4 (License+Qty, tanpa Date): {matched_third} baris")
        st.write(f"   - Tahap 5 (Instruction Number+Qty): {matched_fourth} baris")
        st.write(f"   - **TOTAL MATCHED**: {total_matched} baris")
        st.write(f"   - **TOTAL BARIS FINAL**: {len(df_grpo_timbangan_final)} baris")

        st.success("✓ Semua tahap matching selesai!")
        st.write("=" * 70)
